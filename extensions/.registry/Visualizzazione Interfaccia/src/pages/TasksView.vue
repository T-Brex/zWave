            ...
            <div ref="filterMenuRef" class="filters-button-wrapper">
              ...
              <transition name="filters-pop">
                ...
                <div class="filter-row">
                  ...
                  <div class="filter-row">
                    <label class="filter-label">Immobile</label>
                    <input v-model="filterProperty" class="filter-input" type="text" placeholder="Cerca immobile" />
                  </div>
                </div>
              </transition>
            </div>
          ...
          <div v-if="viewMode === 'table'" class="tasks-table">
            <div class="tasks-table-header">
              ...
              <span class="col-name sortable" @click="toggleSort('name')">
                Nome <span class="sort-indicator">{{ sortIndicator('name') }}</span>
              </span>
              <span class="col-phone sortable" @click="toggleSort('phone')">
                Telefono <span class="sort-indicator">{{ sortIndicator('phone') }}</span>
              </span>
              <span class="col-property sortable" @click="toggleSort('property')">
                Immobile <span class="sort-indicator">{{ sortIndicator('property') }}</span>
              </span>
              <span class="col-date sortable" @click="toggleSort('visitDate')">
                Data <span class="sort-indicator">{{ sortIndicator('visitDate') }}</span>
              </span>
              <span class="col-notes sortable" @click="toggleSort('notes')">
                Note <span class="sort-indicator">{{ sortIndicator('notes') }}</span>
              </span>
              <span></span>
            </div>
            <div
            ...
            >
              ...
              <span class="task-text">{{ task.name || 'N/D' }}</span>
              <span class="task-text">{{ task.phone || 'N/D' }}</span>
              <span class="task-text">
                <span v-if="task.propertyInfo" class="task-subtext">{{ task.propertyInfo }}</span>
                <a
                  v-if="task.propertyLink"
                  :href="task.propertyLink"
                  target="_blank"
                  rel="noopener"
                  class="task-subtext"
                >
                ...
                <span v-if="task.propertyCode" class="task-subtext">{{ task.propertyCode }}</span>
                <span v-if="!task.propertyInfo && !task.propertyLink && !task.propertyCode">N/D</span>
              </span>
              <span class="task-text">{{ task.visitDate || 'N/D' }}</span>
              <span class="task-text">
        ...
        <v-card class="task-dialog">
          ...
          <v-card-text class="task-dialog-content">
            ...
            <div v-if="taskForm.tipo === 'Visita'" class="task-form-row">
              ...
              <input v-model="taskForm.immobile" type="text" placeholder="Info immobile" />
            </div>
            <div class="task-form-row">
              <label>Note</label>
      ...
      <v-dialog v-model="showEditDialog" @esc="closeEditDialog">
        <v-card class="task-dialog">
          ...
          <v-card-text class="task-dialog-content">
            ...
            <div class="task-form-row">
              <label>Immobile</label>
              <input v-model="editForm.immobile" type="text" :disabled="editForm.readonly" />
            </div>
            <div class="task-form-row">
              <label>Note</label>
...
const tagFilters = ref({ richiamare: true, visita: true, altro: true });
...
const filteredTasks = computed(() => {
  ...
  const filtered = tasks.value.filter(task => {
    ...
    if (filterProperty.value.trim()) {
      ...
      const ref = String(task.propertyInfo || '').toLowerCase();
      const link = String(task.propertyLink || '').toLowerCase();
      const code = String(task.propertyCode || '').toLowerCase();
      if (!ref.includes(prop) && !link.includes(prop) && !code.includes(prop)) return false;
    }
    ...
    return (
      ...
      String(task.propertyLink || '').toLowerCase().includes(search) ||
      String(task.propertyCode || '').toLowerCase().includes(search) ||
  ...
  try {
    ...
    tasks.value = [
      ...manualItems
        ...
        .filter(task => ['richiamare', 'visita', 'altro'].includes(task.type))
    ];
  } catch (err) {
...
function normalizeManualTask(item) {
  ...
  const type = rawType === 'richiamare' ? 'richiamare' : rawType === 'visita' ? 'visita' : '';
  return {
    id: item?.id,
    type,
    label: type === 'richiamare' ? 'Richiamare' : 'Visita',
...
function compareByKey(a, b, key, direction) {
  ...
  const getValue = item => {
    ...
    if (key === 'property') {
      return String(item.propertyInfo || item.propertyCode || item.propertyLink || '').toLowerCase();
    }
    if (key === 'visitDate') {
...
async function createTask() {
  ...
  try {
    ...
    if (taskForm.value.tipo === 'Visita') {
      ...
      if (taskForm.value.immobile) {
        payload[MANUAL_TASK_FIELDS.propertyInfo] = taskForm.value.immobile;
      }
    }
...
function openTaskEditor(task) {
  ...
  editForm.value = {
    ...
    tipo: task.type === 'visita' ? 'Visita' : task.type === 'richiamare' ? 'Richiamare' : 'Altro',
    immobile: task.propertyInfo || '',
    note: task.note || '',
    readonly: false,
    allowNotes: true
  };
...
async function saveTaskEdits() {
  ...
  try {
    const payload = {
      ...
      [MANUAL_TASK_FIELDS.propertyInfo]: editForm.value.immobile || ''
    };
...
async function updateTaskTag(task) {
  ...
  const nextValue = task.type === 'visita' ? 'Visita' : 'Richiamare';
  try {
    await updateManualTask(task.id, { [MANUAL_TASK_FIELDS.type]: nextValue });
    task.label = task.type === 'richiamare' ? 'Richiamare' : 'Visita';
  } catch (err) {
    task.type = previousType;
    task.label = previousType === 'richiamare' ? 'Richiamare' : 'Visita';
...
.tasks-row {
  display: grid;
  grid-template-columns: 90px minmax(0, 1.1fr) minmax(0, 1fr) minmax(0, 1.4fr) minmax(0, 0.9fr) minmax(0, 1.2fr) 90px;
  gap: 12px;
  align-items: center;
}
