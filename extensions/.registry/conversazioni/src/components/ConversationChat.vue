  ...
  <div class="chat-container">
    ...
    <div v-else class="chat-wrapper">
      <div class="messages-area" ref="messagesArea">
        <!-- Riassunto -->
        <div
          v-if="getField(activeConversation, 'riassunto')"
          :class="['summary-section', { 'summary-section--collapsed': isSummaryCollapsed }]"
        >
...
function handleScroll() {
  const offset = messagesArea.value?.scrollTop || 0;
  isSummaryCollapsed.value = offset > 80;
}
...
onMounted(() => {
  if (messagesArea.value) {
    messagesArea.value.addEventListener('scroll', handleScroll, { passive: true });
  }
});
