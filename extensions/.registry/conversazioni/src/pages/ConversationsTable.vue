                  ...
                  >
                    ...
                    >
                      <template v-if="column.key === 'details'">
                        <button class="details-link" type="button" @click.stop="goToDetails(conversation)">
                          Dettagli
                        </button>
                      </template>
...
const columns = ref([
  ...
  { key: 'immobile', label: 'Immobile', sortable: false, width: 'auto' },
  { key: 'details', label: 'Dettagli', sortable: false, width: '130px' }
]);
const draggingColumnKey = ref(null);
const tableWidth = computed(() => {
  return columns.value.reduce((sum, column) => {
    const raw = column.width || '160px';
    const parsed = Number.parseInt(raw, 10);
    return sum + (Number.isFinite(parsed) ? parsed : 160);
  }, 0);
});
...
function getCellValueForExport(conversation, columnKey) {
  if (columnKey === 'details') return '';
  if (columnKey === 'data_prenotazione') {
    return formatDateTimeDisplay(getField(conversation, 'data_prenotazione'));
  }
  if (columnKey === 'immobile') {
    return getImmobileValue(conversation).text || '';
  }
  if (columnKey === 'prenotazione_effettuata') {
    return getBookingText(conversation);
  }
  if (columnKey === 'durata_minuti') {
    return getDurationText(conversation);
  }
  if (columnKey === 'date_created') {
    return formatDate(conversation.date_created);
  }
  return getField(conversation, columnKey) ?? '';
}

function exportTable() {
  if (tableConversations.value.length === 0) return;
  exportingTable.value = true;
  try {
    const exportColumns = columns.value.filter(col => col.key !== 'details');
    const headerRow = exportColumns.map(col => escapeCsvCell(col.label)).join(',');
    const dataRows = tableConversations.value.map(conv => {
      return exportColumns
        .map(col => escapeCsvCell(getCellValueForExport(conv, col.key)))
        .join(',');
    });
    ...
    link.href = url;
    const baseName = selectedAzienda.value
      ? `conversazioni_${selectedAzienda.value.replace(/\s+/g, '_')}`
      : 'conversazioni';
